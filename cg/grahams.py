...
class Graham:
    def __init__(self,points):
        ...
        self.LH=Stack()
        for i in self.p:
            while(self.UH.size>0 and orient(self.UH.items[self.UH.size-2],self.UH.items[self.UH.size-1],i)<0):
                self.UH.pop()
            self.UH.push(i)
        for i in reversed(range(len(self.p))):
            while(self.LH.size>0 and orient(self.LH.items[self.LH.size-2],self.LH.items[self.LH.size-1],self.p[i])<0):
                self.LH.pop()
